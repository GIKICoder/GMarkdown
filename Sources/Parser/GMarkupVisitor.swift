...
//        }
    ...
    private func createQuoteAttributes(depth: Int, config: BlockQuoteConfig) -> [NSAttributedString.Key: Any] {
        ...
        paragraphStyle.firstLineHeadIndent = config.baseLeftMargin
        paragraphStyle.headIndent = leftIndent
        paragraphStyle.lineSpacing = 25 - style.fonts.current.pointSize
        paragraphStyle.paragraphSpacing = 16
        attributes[.paragraphStyle] = paragraphStyle
    ...
    private mutating func processChildInBlockQuote(_ child: Markup, attributes: [NSAttributedString.Key: Any]) -> NSMutableAttributedString? {
        guard var childAttributed = visit(child) as? NSMutableAttributedString else { return nil }
        
        childAttributed.enumerateAttribute(.paragraphStyle, in: NSRange(location: 0, length: childAttributed.length), options: []) { _, range, _ in
            if range.location == 0 || (range.location > 0 && childAttributed.string[childAttributed.string.index(before: childAttributed.string.index(childAttributed.string.startIndex, offsetBy: range.location))] == "\n") {
                let quoteMarker = NSAttributedString(string: "   ", attributes: attributes)
                childAttributed.insert(quoteMarker, at: range.location)
            }
        }
