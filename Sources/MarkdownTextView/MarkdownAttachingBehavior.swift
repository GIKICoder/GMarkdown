...
open class MarkdownAttachingBehavior: NSObject {
    ...
    /// 附加视图的缓存映射表
    private let attachedViews = NSMapTable<MarkdownAttachedViewProvider, UIView>.strongToStrongObjects()
    
    /// 当前附加的视图提供者数组
    private var attachedProviders: Array<MarkdownAttachedViewProvider> {
        return Array(self.attachedViews.keyEnumerator()) as! Array<MarkdownAttachedViewProvider>
    }
...
extension MarkdownAttachingBehavior: NSLayoutManagerDelegate {
...
extension MarkdownAttachingBehavior: NSTextStorageDelegate {
...
private extension NSAttributedString {
    ...
    var subviewAttachmentRanges: [(attachment: MarkdownAttachment, range: NSRange)] {
        var ranges = [(MarkdownAttachment, NSRange)]()
